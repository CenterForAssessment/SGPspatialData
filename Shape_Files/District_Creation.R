...
for (i in tmp.unique.indices) {
	...
	if (length(tmp.shp.file.names) > 1) {
		...
		system(paste("topojson -o",  paste(tmp.new.name, "_100_Percent.topojson", sep=""), paste(tmp.new.name, "shp", sep=".")))
		system(paste("topojson -o",  paste(tmp.new.name, "_25_Percent.topojson", sep=""), "--simplify-proportion .25", paste(tmp.new.name, "shp", sep=".")))
	} else {
		if (tmp.abb %in% state.abb) {
